 ...
 * {@link com.mucommander.file.icon.LocalFileIconProvider} and
 * {@link com.mucommander.file.icon.CacheableFileIconProvider} implementations
 * to {@link SwingFileIconProvider}.
 * 
...
class SwingFileIconProviderImpl extends LocalFileIconProvider implements CacheableFileIconProvider {
	...
	/**
	 * Swing object used to retrieve file icons, used on all platforms but Mac
	 * OS X
	 */
	...
	/**
	 * Name of the 'symlink' icon resource located in the same package as this
	 * class
	 */
	private final static String              SYMLINK_ICON_NAME  = "link.png";

	/**
	 * Icon that is painted over a symlink's target file icon to symbolize a
	 * symlink to the target file.
	 */
	...
	/**
	 * Initializes the Swing object used to retrieve icons the first time this
	 * method is called, does nothing subsequent calls. Note: instanciating this
	 * object is expensive (I/O bound) so we want to do that only if needed, and
	 * only once.
	 */
	synchronized static void checkInit() {
		// This method is synchronized to ensure that the initialization happens
		// only once
		if (initialized)
			return;
		...
		// Replace stderr with a SilenceablePrintStream that can be 'silenced'
		// when needed
	...
	/**
	 * Returns an icon for the given <code>java.io.File</code> using the
	 * underlying Swing provider component, <code>null</code> in case of an
	 * error.
	 * 
	 * @param javaIoFile
	 *            the file for which to return an icon
	 * @return an icon for the specified file, null in case of an unexpected
	 *         error
	 */
	private static Icon getSwingIcon(java.io.File javaIoFile) {
		try {
			if (fileSystemView != null) {
				// FileSystemView.getSystemIcon() will behave in the following
				// way if the specified file doesn't exist
				// when the icon is requested:
				// - throw a NullPointerException (caused by a
				// java.io.FileNotFoundException) => OK why not
				// - dump the stack trace to System.err => bad! bad! bad!
				//
				// A way to workaround this odd behavior would be to test if the
				// file exists when it is requested,
				// but a/ this is an expensive operation (especially under
				// Windows) and b/ it wouldn't guarantee that
				// the file effectively exists when the icon is requested.
				// So the workaround here is to catch exceptions and 'silence'
				// System.err output during the call.
	...
	/**
	 * Returns the extension of the given file using
	 * {@link AbstractFile#getExtension()}. If the extension is
	 * <code>null</code>, the empty string <code>""</code> is returned, making
	 * the returned extension safe for use in a hash map where null keys are
	 * forbidden.
	 * 
	 * @param file
	 *            file on which to call {@link AbstractFile#getExtension}
	 * @return the file's extension, may be the empty string but never
	 *         <code>null</code>
	 */
	 ...
	 * <ul>
	 * <li>local files: their icons are cached by the Swing component that
	 * provides icons.</li>
	 * <li>symlinks: their icon cannot be cached using the file's extension as a
	 * key.</li>
	 * </ul>
	 * <code>true</code> is returned for non-local files that are not symlinks
	 * to avoid excessive temporary file creation.
	 */
	...
	public Icon lookupCache(AbstractFile file, Dimension preferredResolution) {
		// Under Mac OS X, return the icon of /Network for the root of remote
		// (non-local) locations.
	...
	/**
	 * <i>Implementation note:</i> only one resolution is available (usually
	 * 16x16) and blindly returned, the <code>preferredResolution</code>
	...
	public Icon getLocalFileIcon(LocalFile localFile, AbstractFile originalFile, Dimension preferredResolution) {
		...
		// - the original file is a local file but the Swing component generates
		// icons which do not have a symlink
		// indication. That is the case on Mac OS X 10.5 (regression, 10.4 did
		// this just fine).
		//
		// Note that the symlink test is performed last because it is the most
		// expensive.
		//
