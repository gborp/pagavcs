...
public class RuntimeConstants {
	 ...
	 * This is guaranteed to trigger a software update - the JAR file is
	 * corrupt, so we might as well get the latest version.
	 */
	...
	static {
		...
		attributes = null;
		try {
			if ((in = ResourceLoader.getResourceAsStream("META-INF/MANIFEST.MF", ResourceLoader.getDefaultClassLoader(), ResourceLoader
			        .getRootPackageAsFile(RuntimeConstants.class))) != null) {
				Manifest manifest;
			...
			if (in != null) {
				try {
					...
					// Ignore this, we don't really care if we can't close this
					// stream.
				}
			}
		}
		...
		if (attributes == null) {
			...
			// We use a date that we are sure is later than the latest version
			// to trigger the version checker.
