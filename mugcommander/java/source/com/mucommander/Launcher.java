...
public class Launcher {
	...
	private static void printUsage() {
		...
		System.out
				.println(" -a FILE, --assoc FILE             Load associations from FILE.");

		// Allows users to tweak how bookmarks are loaded / saved.
		System.out
				.println(" -b FILE, --bookmarks FILE         Load bookmarks from FILE.");

		// Allows users to tweak how configuration is loaded / saved.
		System.out
				.println(" -c FILE, --configuration FILE     Load configuration from FILE");

		// Allows users to tweak how command bar configuration is loaded /
		// saved.
		System.out
				.println(" -C FILE, --commandbar FILE        Load command bar from FILE.");

		// Allows users to change the extensions folder.
		System.out
				.println(" -e FOLDER, --extensions FOLDER    Load extensions from FOLDER.");

		// Allows users to tweak how custom commands are loaded / saved.
		System.out
				.println(" -f FILE, --commands FILE          Load custom commands from FILE.");

		// Ignore warnings.
		System.out
				.println(" -i, --ignore-warnings             Do not fail on warnings (default).");

		// Allows users to tweak how keymaps are loaded.
		System.out
				.println(" -k FILE, --keymap FILE            Load keymap from FILE");

		// Allows users to change the preferences folder.
		System.out
				.println(" -p FOLDER, --preferences FOLDER   Store configuration files in FOLDER");

		// muCommander will not print verbose error messages.
		System.out
				.println(" -S, --silent                      Do not print verbose error messages");

		// Allows users to tweak how shell history is loaded / saved.
		System.out
				.println(" -s FILE, --shell-history FILE     Load shell history from FILE");

		// Allows users to tweak how toolbar configuration are loaded.
		System.out
				.println(" -t FILE, --toolbar FILE           Load toolbar from FILE");

		// Allows users to tweak how credentials are loaded.
		System.out
				.println(" -u FILE, --credentials FILE       Load credentials from FILE");

		// Text commands.
		System.out
				.println(" -h, --help                        Print the help text and exit");
		System.out
				.println(" -v, --version                     Print the version and exit");

		// muCommander will print verbose boot error messages.
		System.out
				.println(" -V, --verbose                     Print verbose error messages (default)");

		// Pedantic mode.
		System.out
				.println(" -w, --fail-on-warnings            Quits when a warning is encountered during");
		System.out
				.println("                                   the boot process.");
		System.exit(0);
	}
	...
	private static void printVersion() {
		...
		System.out
				.println("This is free software, distributed under the terms of the GNU General Public License.");
		System.exit(0);
	}
	 ...
	 * 
	 * @param msg
	 *            error message to print to stder.
	 * @param quit
	 ...
	 */
	private static StringBuffer createErrorMessage(String msg,
			Exception exception, boolean quit) {
		StringBuffer error;
	 ...
	 */
	private static void printFileError(String msg, Exception exception,
			boolean quit) {
		StringBuffer error;
	 ...
	 *            variable.
	 */
	private static void migrateCommand(String useName, String commandName,
			String alias) {
		String command;
		...
		if (MuConfiguration.getBooleanVariable(useName)
			...
			try {
				CommandManager.registerCommand(new Command(alias, command,
						Command.SYSTEM_COMMAND));
			} catch (CommandException e) {
				// Ignore this: the command didn't work in the first place, we
		...
		try {
			...
			for (i = 0; i < args.length; i++) {
				...
				// Associations handling.
					...
					try {
						com.mucommander.command.CommandManager
								.setAssociationFile(args[++i]);
					} catch (Exception e) {
						printError("Could not set association files", e,
								fatalWarnings);
					}
				}
				...
				// Custom commands handling.
					...
					try {
						com.mucommander.command.CommandManager
								.setCommandFile(args[++i]);
					} catch (Exception e) {
						printError("Could not set commands file", e,
								fatalWarnings);
					}
				}
				...
				// Bookmarks handling.
					...
					if (i >= args.length - 1)
						printError("Missing FILE parameter to " + args[i],
								null, true);
					try {
						com.mucommander.bookmark.BookmarkManager
								.setBookmarksFile(args[++i]);
					} catch (Exception e) {
						printError("Could not set bookmarks file", e,
								fatalWarnings);
					}
				}
				...
				// Configuration handling.
					...
					try {
						...
						printError("Could not set configuration file", e,
								fatalWarnings);
					}
				}
				...
				// Shell history.
					...
					try {
						...
						printError("Could not set shell history file", e,
								fatalWarnings);
					}
				}
				...
				// Keymap file.
					...
					try {
						com.mucommander.ui.action.ActionKeymapIO
								.setActionsFile(args[++i]);
					} catch (Exception e) {
						printError("Could not set keymap file", e,
								fatalWarnings);
					}
				}
				...
				// Commandbar file.
					...
					try {
						...
						printError("Could not set commandbar description file",
								e, fatalWarnings);
					}
				}
				...
				// Credentials file.
						...
						|| args[i].equals("--credentials")) {
					if (i >= args.length - 1)
						printError("Missing FILE parameter to " + args[i],
								null, true);
					try {
						com.mucommander.auth.CredentialsManager
								.setCredentialsFile(args[++i]);
					} catch (Exception e) {
						printError("Could not set credentials file", e,
								fatalWarnings);
					}
				}

				// Preference folder.
				else if ((args[i].equals("-p") || args[i]
						.equals("--preferences"))) {
					if (i >= args.length - 1)
					...
					try {
						...
						printError("Could not set preferences folder", e,
								fatalWarnings);
					}
				}
				...
				// Extensions folder.
					...
					try {
						...
						printError("Could not set extensions folder", e,
								fatalWarnings);
					}
				}
				...
				// Ignore warnings.
						...
						|| args[i].equals("--ignore-warnings"))
					fatalWarnings = false;

				// Fail on warnings.
				else if (args[i].equals("-w")
						|| args[i].equals("--fail-on-warnings"))
					fatalWarnings = true;
			...
			if (OsFamilies.MAC_OS_X.isCurrent()) {
				...
				// other than Mac OS X
				try {
					Class<?> osxIntegrationClass = Class
							.forName("com.mucommander.ui.macosx.OSXIntegration");
					Constructor<?> constructor = osxIntegrationClass
							.getConstructor(new Class[] {});
					constructor.newInstance();
				} catch (Exception e) {
					AppLogger
							.fine("Exception thrown while initializing Mac OS X integration",
									e);
				}
			}
			...
			useSplash = MuConfiguration.getVariable(
					MuConfiguration.SHOW_SPLASH_SCREEN,
					MuConfiguration.DEFAULT_SHOW_SPLASH_SCREEN);
			if (useSplash) {
				splashScreen = new SplashScreen(RuntimeConstants.VERSION,
						"Loading preferences...");
			}
			...
			try {
				...
				printFileError("Could not load custom commands", e,
						fatalWarnings);
			}

			// Migrates the custom editor and custom viewer if necessary.
			migrateCommand("viewer.use_custom", "viewer.custom_command",
					CommandManager.VIEWER_ALIAS);
			migrateCommand("editor.use_custom", "editor.custom_command",
					CommandManager.EDITOR_ALIAS);
			try {
			...
			try {
				...
				printFileError("Could not load custom associations", e,
						fatalWarnings);
			}
			...
			com.mucommander.ui.icon.FileIcons.setScaleFactor(Math.max(1.0f,
					MuConfiguration.getVariable(
							MuConfiguration.TABLE_ICON_SCALE,
							MuConfiguration.DEFAULT_TABLE_ICON_SCALE)));
			com.mucommander.ui.icon.FileIcons
					.setSystemIconsPolicy(MuConfiguration.getVariable(
			...
			try {
				...
				printFileError("Could not load actions shortcuts", e,
						fatalWarnings);
			}
			...
			try {
				...
				printFileError("Could not load toolbar description", e,
						fatalWarnings);
			}
			...
			try {
				...
				printFileError("Could not load commandbar description", e,
						fatalWarnings);
			}
			...
			for (ServiceExtension ext : DynamicExtensionsManager.getInstance()
					.getLstServiceExtensions()) {
				ext.startService(useSplash, splashScreen);
			}

			// Creates the initial main frame using any initial path specified
			// by the command line.
			...
			// launched.
			// Important: this must be done before disposing the splash screen,
			// as this would otherwise create a deadlock
			...
			if (MuConfiguration.getVariable(
				...
				if (com.mucommander.ui.notifier.AbstractNotifier.isAvailable())
					com.mucommander.ui.notifier.AbstractNotifier.getNotifier()
							.setEnabled(true);
			}
			...
			// If no theme is configured in the preferences, ask for an initial
			// theme.
			if (showSetup)
				new InitialSetupDialog(WindowManager.getCurrentMainFrame())
						.showDialog();
		} catch (Throwable t) {
			...
			InformationDialog.showErrorDialog(null, null,
					Translator.get("startup_error"), null, t);
	...
	private static void configureFilesystems() {
		...
		// Mac OS X Leopard.
		// Since jCIFS 1.3.0, the default is to use NTLM v2 authentication and
		// extended security.
		SMBProtocolProvider.setLmCompatibility(MuConfiguration.getVariable(
				MuConfiguration.SMB_LM_COMPATIBILITY,
				MuConfiguration.DEFAULT_SMB_LM_COMPATIBILITY));
		SMBProtocolProvider.setExtendedSecurity(MuConfiguration.getVariable(
				MuConfiguration.SMB_USE_EXTENDED_SECURITY,
				MuConfiguration.DEFAULT_SMB_USE_EXTENDED_SECURITY));

		// Use the FTP configuration option that controls whether to force the
		// display of hidden files, or leave it for
		// the servers to decide whether to show them.
		FTPProtocolProvider.setForceHiddenFilesListing(MuConfiguration
				.getVariable(MuConfiguration.LIST_HIDDEN_FILES,
						MuConfiguration.DEFAULT_LIST_HIDDEN_FILES));
	...
	private static void configureLogging() throws IOException {
		// We're no longer using LogManager and a logging.properties file to
		// initialize java.util.logging, because of
		// a limitation with Webstart limiting the use of handlers and
		// formatters residing in the system's classpath,
		// i.e. built-in ones.

		// // Read the java.util.logging configuration file bundled with the
		// muCommander JAR, replacing the JRE's
		...
		// and add ours
		handlers = new Handler[] { new ConsoleHandler(),
				new DebugConsoleHandler() };
	...
	/**
	 * Returns the current log level used by all <code>java.util.logging</code>
	 * loggers.
	 * 
	 * @return the current log level used by all <code>java.util.logging</code>
	 *         loggers.
	 */
	public static Level getLogLevel() {
		return Level.parse(MuConfiguration.getVariable(
				MuConfiguration.LOG_LEVEL, MuConfiguration.DEFAULT_LOG_LEVEL));
	}

	/**
	 * Sets the new log level to be used by all <code>java.util.logging</code>
	 * loggers, and persists it in the application preferences.
	 * 
	 * @param level
	 *            the new log level to be used by all
	 *            <code>java.util.logging</code> loggers.
	 */
	...
	/**
	 * Sets the level of all muCommander loggers, namely {@link AppLogger},
	 * {@link FileLogger} and {@link CommonsLogger}.
	 * 
	 * @param level
	 *            the new log level
	 */
	public static void updateLogLevel(Level level) {
		...
		// Note: non-muCommander loggers default to the level defined in the
		// JRE's logging.properties.
