...
/**
 * This class represents a major version of an operating system, like
 * <code>Mac OS X 10.5</code> or <code>Windows XP</code>. The current runtime
 * value is determined using the value of the <code>os.version</code> system
 * property and the current {@link OsFamily} instance. Being a
 * {@link com.mucommander.runtime.ComparableRuntimeProperty}, OS versions are
 * ordered and can be compared against each other.
 * 
 * @see OsVersions
 * @see OsFamily
...
public class OsVersion extends ComparableRuntimeProperty implements OsVersions {
	...
	/**
	 * Determines the current value by parsing the corresponding system
	 * property. This method is called automatically by this class the first
	 * time the current value is accessed. However, this method has been made
	 * public to allow to force the initialization if it needs to happen at a
	 * predictable time.
	 */
	public static void init() {
		// Note: performing the initialization outside of the class static block
		// avoids cyclic dependency problems.
		if (currentValue == null) {
			currentValue = parseSystemProperty(getRawSystemProperty(),
					OsFamily.getRawSystemProperty(), OsFamily.getCurrent());
	...
	/**
	 * Returns the value of the system property which serves to detect the OS
	 * version at runtime.
	 * 
	 * @return the value of the system property which serves to detect the OS
	 *         version at runtime.
	 */
	...
	/**
	 * Returns an <code>OsVersion</code> instance corresponding to the specified
	 ...
	 * @return an OsVersion instance corresponding to the specified system
	 *         property's value
	 */
	static OsVersion parseSystemProperty(String osVersionProp,
			String osNameProp, OsFamily osFamily) {
		OsVersion osVersion;

		// This website holds a collection of system property values under many
		// OSes:
		...
		if (osFamily == OsFamilies.MAC_OS_X) {
			if (osVersionProp.startsWith("10.6")) {
				...
				// Newer version we don't know of yet, assume latest supported
				// OS version
				osVersion = MAC_OS_X_10_6;
			}
		} else {
