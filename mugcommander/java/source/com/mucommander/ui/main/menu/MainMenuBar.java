...
/**
 * This class is the main menu bar. It takes care of displaying menu and menu
 * items and triggering the proper actions.
 * <p>
 * <b>Implementation note</b>: for performance reasons, some menu items are
 * created/enabled/disabled when corresponding menus are selected, instead of
 * monitoring the MainFrame's state and unnecessarily
 * creating/enabling/disabling menu items when they are not visible. However,
 * this prevents keyboard shortcuts from being managed by the menu bar for those
 * dynamic items.
 * 
...
public class MainMenuBar extends JMenuBar implements ActionListener, MenuListener {
	...
	private int                           bookmarksOffset;                                                           // Index
																													  // of
																													  // the
																													  // first
																													  // bookmark
	...
	private int                           windowOffset;                                                              // Index
																													  // of
																													  // the
																													  // first
																													  // window
	...
	private final static String           RECALL_WINDOW_ACTION_IDS[] = { com.mucommander.ui.action.impl.RecallWindow1Action.Descriptor.ACTION_ID,
	        com.mucommander.ui.action.impl.RecallWindow2Action.Descriptor.ACTION_ID, com.mucommander.ui.action.impl.RecallWindow3Action.Descriptor.ACTION_ID,
	        com.mucommander.ui.action.impl.RecallWindow4Action.Descriptor.ACTION_ID, com.mucommander.ui.action.impl.RecallWindow5Action.Descriptor.ACTION_ID,
	        com.mucommander.ui.action.impl.RecallWindow6Action.Descriptor.ACTION_ID, com.mucommander.ui.action.impl.RecallWindow7Action.Descriptor.ACTION_ID,
	        com.mucommander.ui.action.impl.RecallWindow8Action.Descriptor.ACTION_ID, com.mucommander.ui.action.impl.RecallWindow9Action.Descriptor.ACTION_ID,
	...
	public MainMenuBar(MainFrame mainFrame) {
		...
		// Disable menu bar (NOT menu item) mnemonics under Mac OS X because of
		// a bug: when screen menu bar is enabled
		// and a menu is triggered by a mnemonic, the menu pops up where it
		// would appear with a regular menu bar
		...
		// Under Mac OS X, 'Preferences' already appears in the application
		...
		// Under Mac OS X, 'Quit' already appears in the application
		// (muCommander) menu, do not display it again
		if (!OsFamilies.MAC_OS_X.isCurrent())
		...
		toggleShowFoldersFirstItem = MenuToolkit.addCheckBoxMenuItem(viewMenu, ActionManager.getActionInstance(
		        ToggleShowFoldersFirstAction.Descriptor.ACTION_ID, mainFrame), menuItemMnemonicHelper);
		toggleShowHiddenFilesItem = MenuToolkit.addCheckBoxMenuItem(viewMenu, ActionManager.getActionInstance(ToggleHiddenFilesAction.Descriptor.ACTION_ID,
		        mainFrame), menuItemMnemonicHelper);
		toggleTreeItem = MenuToolkit.addCheckBoxMenuItem(viewMenu, ActionManager.getActionInstance(ToggleTreeAction.Descriptor.ACTION_ID, mainFrame),
		        menuItemMnemonicHelper);
		/*
		 * TODO branch toggleBranchView =
		 * MenuToolkit.addCheckBoxMenuItem(viewMenu,
		 * ActionManager.getActionInstance(ToggleBranchViewAction.class,
		 * mainFrame), menuItemMnemonicHelper);
		 */
		...
		for (Column c : Column.values())
			buttonGroup.add(sortByItems[c.ordinal()] = MenuToolkit.addCheckBoxMenuItem(viewMenu, ActionManager.getActionInstance(c.getSortByColumnActionId(),
			        mainFrame), menuItemMnemonicHelper));
		...
		for (Column c : Column.values()) {
			...
			toggleColumnItems[c.ordinal()] = MenuToolkit.addCheckBoxMenuItem(columnsMenu, ActionManager.getActionInstance(c.getToggleColumnActionId(),
			        mainFrame), menuItemMnemonicHelper2);
		}
		viewMenu.add(columnsMenu);

		toggleToggleAutoSizeItem = MenuToolkit.addCheckBoxMenuItem(viewMenu, ActionManager.getActionInstance(ToggleAutoSizeAction.Descriptor.ACTION_ID,
		        mainFrame), menuItemMnemonicHelper);
		...
		menuItemMnemonicHelper2.clear();
		MenuToolkit.addMenuItem(quickListMenu, ActionManager.getActionInstance(ShowParentFoldersQLAction.Descriptor.ACTION_ID, mainFrame),
		        menuItemMnemonicHelper2);
		MenuToolkit.addMenuItem(quickListMenu, ActionManager.getActionInstance(ShowRecentLocationsQLAction.Descriptor.ACTION_ID, mainFrame),
		        menuItemMnemonicHelper2);
		MenuToolkit.addMenuItem(quickListMenu, ActionManager.getActionInstance(ShowRecentExecutedFilesQLAction.Descriptor.ACTION_ID, mainFrame),
		        menuItemMnemonicHelper2);
		...
		// Save the first bookmark menu item's offset for later (bookmarks will
		// be added when menu becomes visible)
		...
		buttonGroup.add(splitVerticallyItem = MenuToolkit.addCheckBoxMenuItem(windowMenu, ActionManager.getActionInstance(
		        SplitVerticallyAction.Descriptor.ACTION_ID, mainFrame), menuItemMnemonicHelper));
		buttonGroup.add(splitHorizontallyItem = MenuToolkit.addCheckBoxMenuItem(windowMenu, ActionManager.getActionInstance(
		        SplitHorizontallyAction.Descriptor.ACTION_ID, mainFrame), menuItemMnemonicHelper));
		...
		windowMenu.add(new JSeparator());
		MenuToolkit
		        .addMenuItem(windowMenu, ActionManager.getActionInstance(RecallPreviousWindowAction.Descriptor.ACTION_ID, mainFrame), menuItemMnemonicHelper);
		...
		// Links to website, only shows for OS/Window manager that can launch
		// the default browser to open URLs
	...
	public void menuSelected(MenuEvent e) {
		...
		if (source == viewMenu) {
			...
			// Select the 'sort by' criterion currently in use in the active
			// table
			...
			/*
			 * TODO branch
			 * toggleBranchView.setSelected(activeTable.getFolderPanel
			 * ().isBranchView());
			 */
			...
			FileTable activeTable = mainFrame.getActiveTable();
			for (Column c : Column.values()) {
				if (c == Column.NAME) // Name column doesn't have a menu item as
			...
			if (nbBookmarks > 0) {
				...
				// Show 'No bookmark' as a disabled menu item instead showing
				// nothing
			...
			// Note: menu item cannot be removed by menuDeselected() as
			// actionPerformed() will be called after
			// menu has been deselected.
			for (int i = windowMenu.getItemCount(); i > windowOffset; i--)
				windowMenu.remove(windowOffset);

			// This WeakHashMap maps menu items to frame instances. It has to be
			// a weakly referenced hash map
			// and not a regular hash map, since it will not (and cannot) be
			// emptied when the menu has been deselected
			// and we really do not want this hash map to prevent the frames to
			// be GCed
			windowMenuFrames = new WeakHashMap<JMenuItem, Frame>();

			// Create a menu item for each of the MainFrame instances, that
			// displays the MainFrame's path
			// and a keyboard accelerator to recall the frame (for the first 10
			// frames only).
			...
			for (int i = 0; i < nbFrames; i++) {
				...
				// If frame number is less than 10, use the corresponding action
				// class (accelerator will be displayed in the menu item)
				...
				// Else use the generic RecallWindowAction
					...
					recallWindowAction = ActionManager.getActionInstance(new ActionParameters(RecallWindowAction.Descriptor.ACTION_ID, actionProps),
					        this.mainFrame);
				}

				checkBoxMenuItem.setAction(recallWindowAction);

				// Replace the action's label and use the MainFrame's current
				// folder path instead
			...
			for (int i = 0; i < nbFrames; i++) {
				frame = frames[i];
				// Test if Frame is not hidden (disposed), Frame.getFrames()
				// returns both active and disposed frames
				if (frame.isShowing() && ((frame instanceof ViewerFrame) || (frame instanceof EditorFrame))) {
					// Add a separator before the first non-MainFrame frame to
					// mark a separation between MainFrames
					// and other frames
					if (firstFrame) {
						windowMenu.add(new JSeparator());
						firstFrame = false;
			...
			// Remove all previous theme items, create new ones for each
			// available theme and select the current theme
	...
	/**
	 * Action that changes the current theme to the specified in the
	 * constructor.
	 */
