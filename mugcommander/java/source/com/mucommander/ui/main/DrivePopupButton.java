...
/**
 * <code>DrivePopupButton</code> is a button which, when clicked, pops up a menu
 * with a list of volumes items that be used to change the current folder.
 * 
 * @author Maxence Bernard
 */
public class DrivePopupButton extends PopupButton implements LocationListener,
		BookmarkListener, ConfigurationListener {
	...
	/**
	 * Filters out volumes from the list based on the exclude regexp defined in
	 * the configuration, null if the regexp is not defined.
	 */
	...
	static {
		...
		try {
			String excludeRegexp = MuConfiguration
					.getVariable(MuConfiguration.VOLUME_EXCLUDE_REGEXP);
			if (excludeRegexp != null) {
				volumeFilter = new RegexpPathFilter(excludeRegexp, true);
				volumeFilter.setInverted(true);
			}
		} catch (PatternSyntaxException e) {
			AppLogger.info("Invalid regexp for conf variable "
					+ MuConfiguration.VOLUME_EXCLUDE_REGEXP, e);
		}
	...
	/**
	 * Creates a new <code>DrivePopupButton</code> which is to be added to the
	 * given FolderPanel.
	 * 
	 * @param folderPanel
	...
	public DrivePopupButton(FolderPanel folderPanel) {
		...
		// Listen to location events to update the button when the current
		// folder changes
		folderPanel.getLocationManager().addLocationListener(this);

		// Listen to bookmark changes to update the button if a bookmark
		// corresponding to the current folder
		// has been added/edited/removed
		BookmarkManager.addBookmarkListener(this);

		// Listen to configuration changes to update the button if the system
		// file icons policy has changed
	...
	/**
	 * Updates the button's label and icon to reflect the current folder and
	 * match one of the current volumes: <
	 * <ul>
	 * <li>If the specified folder corresponds to a bookmark, the bookmark's
	 * name will be displayed
	 * <li>If the specified folder corresponds to a local file, the enclosing
	 * volume's name will be displayed
	 * <li>If the specified folder corresponds to a remote file, the protocol's
	 * name will be displayed
	 * </ul>
	...
	private void updateButton() {
		...
		for (int i = 0; i < nbBookmarks; i++) {
			...
			if (currentPath.equals(b.getLocation())) {
				// Note: if several bookmarks match current folder, the first
				// one will be used
				newLabel = b.getName();
				break;
			}
		...
		if (newLabel == null) {
			...
			// Local file, use volume's name
				...
				// getCanonicalPath() must be avoided under Windows for the
				// following reasons:
				// a) it is not necessary, Windows doesn't have symlinks
				// b) it triggers the dreaded 'No disk in drive' error popup
				// dialog.
				// c) when network drives are present but not mounted (e.g.
				// X:\ mapped onto an SMB share),
				// getCanonicalPath which is I/O bound will take a looooong
				// time to execute

				currentPath = currentFolder.getCanonicalPath(false)
						.toLowerCase();
				...
				// newToolTip =
				// getWindowsExtendedDriveName(volumes[bestIndex]);
			}
		}
	...
	/**
	 * Returns the extended name of the given local file, e.g. "Local Disk (C:)"
	 * for C:\. The returned value is interesting only under Windows. This
	 * method is I/O bound and very slow so it should not be called from the
	 * main event thread.
	 * 
	 * @param localFile
	...
	private static String getExtendedDriveName(AbstractFile localFile) {
		// Note: fileSystemView.getSystemDisplayName(java.io.File) is
		// unfortunately very very slow
		String name = fileSystemView
				.getSystemDisplayName((java.io.File) localFile
						.getUnderlyingFileObject());

		if (name == null || name.equals("")) // This happens for CD/DVD drives
	 ...
	 * 
	 * <p>
	 * The raw list of volumes is fetched using {@link LocalFile#getVolumes()}
	 * and then filtered using the regexp defined in the
	 * {@link MuConfiguration#VOLUME_EXCLUDE_REGEXP} configuration variable (if
	 * defined).
	 * </p>
	 * 
	...
	public JPopupMenu getPopupMenu() {
		...
		MnemonicHelper mnemonicHelper = new MnemonicHelper(); // Provides
																// mnemonics and
																// ensures
																// uniqueness
		JMenuItem item;
		...
		for (int i = 0; i < nbVolumes; i++) {
			action = new CustomOpenLocationAction(folderPanel, mainFrame,
					new Hashtable<String, Object>(), volumes[i]);
			volumeName = volumes[i].getName();

			// If several volumes have the same filename, use the volume's path
			// for the action's label instead of the
			...
			if (useExtendedDriveNames) {
				// Use the last known value (if any) while we update it in a
				// separate thread
			...
			itemsV.add(item); // JMenu offers no way to retrieve a particular
								// JMenuItem, so we have to keep them
		}
		...
		if (nbBookmarks > 0) {
			for (int i = 0; i < nbBookmarks; i++) {
				b = bookmarks.elementAt(i);
				item = popupMenu.add(new CustomOpenLocationAction(folderPanel,
						mainFrame, new Hashtable<String, Object>(), b));
				setMnemonic(item, mnemonicHelper);
			}
		} else {
			// No bookmark : add a disabled menu item saying there is no
			// bookmark
			popupMenu.add(Translator.get("bookmarks_menu.no_bookmark"))
					.setEnabled(false);
		}
		...
		if (FileFactory.isRegisteredProtocol(FileProtocols.SMB)) {
			action = new CustomOpenLocationAction(folderPanel, mainFrame,
					new Hashtable<String, Object>(), new Bookmark(
							Translator.get("drive_popup.network_shares"),
							"smb:///"));
			action.setIcon(IconManager.getIcon(IconManager.FILE_ICON_SET,
					CustomFileIconProvider.NETWORK_ICON_NAME));
			setMnemonic(popupMenu.add(action), mnemonicHelper);
		}

		// Add extension shortcuts (eg.: bonjour)
		for (DrivePopupMenuExtension extension : DynamicExtensionsManager
				.getInstance().getLstDrivePopupMenuExtensions()) {
			setMnemonic(popupMenu.add(extension.getDrivePopupMenuItem(
					folderPanel, mainFrame)), mnemonicHelper);
		}

		// Add 'connect to server' shortcuts
		setMnemonic(popupMenu.add(new ServerConnectAction("SMB...",
				SMBPanel.class)), mnemonicHelper);
		setMnemonic(popupMenu.add(new ServerConnectAction("FTP...",
				FTPPanel.class)), mnemonicHelper);
		setMnemonic(popupMenu.add(new ServerConnectAction("SFTP...",
				SFTPPanel.class)), mnemonicHelper);
		setMnemonic(popupMenu.add(new ServerConnectAction("HTTP...",
				HTTPPanel.class)), mnemonicHelper);
		setMnemonic(popupMenu.add(new ServerConnectAction("NFS...",
				NFSPanel.class)), mnemonicHelper);

		return popupMenu;
	}

	/**
	 * Calls to getExtendedDriveName(String) are very slow, so they are
	 * performed in a separate thread so as to not lock the main even thread.
	 * The popup menu gets first displayed with the short drive names, and then
	 * refreshed with the extended names as they are retrieved.
	 */
	private class RefreshDriveNamesAndIcons extends Thread {
		...
		public RefreshDriveNamesAndIcons(JPopupMenu popupMenu,
		...
		public void run() {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (useExtendedDriveNames) {
					// Under Windows, show the extended drive name (e.g.
					// "Local Disk (C:)" instead of just "C:") but use
					// the simple drive name for the mnemonic (i.e. 'C' instead
					// of 'L').
				...
				// Set system icon for volumes, only if system icons are
				// available on the current platform
				final Icon icon = FileIcons.hasProperSystemIcons() ? FileIcons
						.getSystemFileIcon(volumes[i]) : null;
				if (icon != null) {
	...
	/**
	 * Convenience method that sets a mnemonic to the given JMenuItem, using the
	 * specified MnemonicHelper.
	 * 
	 * @param menuItem
	 *            the menu item for which to set a mnemonic
	 * @param mnemonicHelper
	 *            the MnemonicHelper instance to be used to determine the
	 *            mnemonic's character.
	 */
	...
	public void bookmarksChanged() {
		// Refresh label in case a bookmark with the current location was
		// changed
		updateButton();
	}
	...
	public Dimension getPreferredSize() {
		// Limit button's maximum width to something reasonable and leave enough
		// space for location field,
		// as bookmarks name can be as long as users want them to be.
		// Note: would be better to use JButton.setMaximumSize() but it doesn't
		// seem to work
	...
	/**
	 * This action pops up
	 * {@link com.mucommander.ui.dialog.server.ServerConnectDialog} for a
	 * specified protocol.
	 */
	private class ServerConnectAction extends AbstractAction {
		private Class<? extends ServerPanel> serverPanelClass;

		private ServerConnectAction(String label,
				Class<? extends ServerPanel> serverPanelClass) {
			super(label);
	...
	/**
	 * This modified {@link OpenLocationAction} changes the current folder on
	 * the {@link FolderPanel} that contains this button, instead of the
	 * currently active {@link FolderPanel}.
	 */
	public static class CustomOpenLocationAction extends OpenLocationAction {

		private final FolderPanel folderPanel;

		public CustomOpenLocationAction(FolderPanel folderPanel,
				MainFrame mainFrame, Hashtable<String, Object> properties,
				Bookmark bookmark) {
		...
		public CustomOpenLocationAction(FolderPanel folderPanel,
				MainFrame mainFrame, Hashtable<String, Object> hashtable,
				FileURL url, String nameWithProtocol) {
