...
/**
 * This is the main frame, which contains all other UI components visible on a
 * mucommander window.
 * 
...
public class MainFrame extends JFrame implements LocationListener {
	...
	/**
	 * Contains all registered ActivePanelListener instances, stored as weak
	 * references
	 */
	...
	/**
	 * Sets the window icon, using the best method (Java 1.6's
	 * Window#setIconImages when available, Window#setIconImage otherwise) and
	 * icon resolution(s) (OS-dependent).
	 */
	private void setWindowIcon() {
		// TODO: this code should probably be moved to the desktop API

		// - Mac OS X completely ignores calls to #setIconImage/setIconImages,
	...
	private void init(FolderPanel leftFolderPanel, FolderPanel rightFolderPanel) {
		...
		// The toolbar should have no inset, this is why it is left out of the
		// insetsPane
		...
		// Create the toolbar and corresponding panel wrapping it, and show it
		// only if it hasn't been disabled in the
		// preferences.
		// Note: Toolbar.setVisible() has to be called no matter if Toolbar is
		// visible or not, in order for it to be
		// properly initialized
		this.toolbar = new ToolBar(this);
		this.toolbarPanel = ToolbarMoreButton.wrapToolBar(toolbar);
		this.toolbarPanel.setVisible(MuConfiguration.getVariable(
				MuConfiguration.TOOLBAR_VISIBLE,
		...
		// Lister to location change events to display the current folder in the
		// window's title
		...
		// Create the split pane that separates folder panels and allows to
		// resize how much space is allocated to the
		// both of them. The split orientation is loaded from and saved to the
		// preferences.
		// Note: the vertical/horizontal terminology used in muCommander is just
		// the opposite of the one used
		// in JSplitPane which is anti-natural / confusing.
		splitPane = new ProportionalSplitPane(
				this,
				MuConfiguration.getVariable(SPLIT_ORIENTATION,
						MuConfiguration.DEFAULT_SPLIT_ORIENTATION).equals(
						MuConfiguration.VERTICAL_SPLIT_ORIENTATION) ? JSplitPane.HORIZONTAL_SPLIT
						: JSplitPane.VERTICAL_SPLIT, false,
				MainFrame.this.leftFolderPanel, MainFrame.this.rightFolderPanel) {
		...
		// Adds buttons that allow to collapse and expand the split pane in both
		// directions
		splitPane.setOneTouchExpandable(true);

		// Disable all the JSPlitPane accessibility shortcuts that are
		// registered by default, as some of them
		...
		// Note: CommandBar.setVisible() has to be called no matter if
		// CommandBar is visible or not, in order for it to be properly
		// initialized
		this.commandBar.setVisible(MuConfiguration.getVariable(
				MuConfiguration.COMMAND_BAR_VISIBLE,
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				ActionManager.performAction(
						CloseWindowAction.Descriptor.ACTION_ID, MainFrame.this);
			}
		});
		...
		// to notify of the intial active table.
		fireActivePanelChanged(activeTable.getFolderPanel());

		// Set the custom FocusTraversalPolicy that manages focus for both
		// FolderPanel and their subcomponents.
	...
	private FileTableConfiguration getFileTableConfiguration(boolean isLeft) {
		...
		for (Column c : Column.values()) {
			if (c != Column.NAME) { // Skip the special name column (always
									// visible, width automatically calculated)
				// Sets the column's initial visibility.
				conf.setEnabled(c, MuConfiguration.getVariable(
						MuConfiguration.getShowColumnVariable(c, isLeft),
						c.showByDefault()));

				// Sets the column's initial width.
				conf.setWidth(c, MuConfiguration
						.getIntegerVariable(MuConfiguration
								.getColumnWidthVariable(c, isLeft)));
			}
	 ...
	 * 
	 * @param leftInitialFolder
	 *            the initial folder to display in the left panel
	 * @param rightInitialFolder
	 *            the initial folder to display in the right panel
	 */
	public MainFrame(AbstractFile leftInitialFolder,
			AbstractFile rightInitialFolder) {
		init(new FolderPanel(this, leftInitialFolder,
				getFileTableConfiguration(true)), new FolderPanel(this,
				rightInitialFolder, getFileTableConfiguration(false)));

		leftTable.sortBy(
				Column.valueOf(MuConfiguration.getVariable(
						MuConfiguration.LEFT_SORT_BY,
						MuConfiguration.DEFAULT_SORT_BY).toUpperCase()),
				!MuConfiguration.getVariable(MuConfiguration.LEFT_SORT_ORDER,
						MuConfiguration.DEFAULT_SORT_ORDER).equals(
						MuConfiguration.SORT_ORDER_DESCENDING));
		rightTable.sortBy(
				Column.valueOf(MuConfiguration.getVariable(
						MuConfiguration.RIGHT_SORT_BY,
						MuConfiguration.DEFAULT_SORT_BY).toUpperCase()),
				!MuConfiguration.getVariable(MuConfiguration.RIGHT_SORT_ORDER,
						MuConfiguration.DEFAULT_SORT_ORDER).equals(
						MuConfiguration.SORT_ORDER_DESCENDING));
		leftFolderPanel.setTreeWidth(MuConfiguration.getVariable(
				MuConfiguration.LEFT_TREE_WIDTH, 150));
		if (MuConfiguration.getVariable(MuConfiguration.LEFT_TREE_VISIBLE,
				false)) {
			leftFolderPanel.setTreeVisible(true);
		}
		rightFolderPanel.setTreeWidth(MuConfiguration.getVariable(
				MuConfiguration.RIGHT_TREE_WIDTH, 150));
		if (MuConfiguration.getVariable(MuConfiguration.RIGHT_TREE_VISIBLE,
				false)) {
	...
	MainFrame cloneMainFrame() {
		...
		mainFrame.init(
				new FolderPanel(mainFrame, leftFolderPanel.getCurrentFolder(),
						leftTable.getConfiguration()), new FolderPanel(
						mainFrame, rightFolderPanel.getCurrentFolder(),
						rightTable.getConfiguration()));
	...
	/**
	 * Registers the given ActivePanelListener to receive events when the active
	 * table changes.
	 * 
	 * @param activePanelListener
	 *            the ActivePanelListener to add
	 */
	...
	/**
	 * Unregisters the given ActivePanelListener so that it no longer receives
	 * events when the active table changes.
	 * 
	 * @param activePanelListener
	 *            the ActivePanelListener to remove
	 */
	public void removeActivePanelListener(
			ActivePanelListener activePanelListener) {
		activePanelListeners.remove(activePanelListener);
	}

	/**
	 * Fires table change events on all registered ActivePanelListener
	 * instances.
	 * 
	 * @param folderPanel
	...
	/**
	 * Enables/disables the 'no events mode' which prevents mouse and keyboard
	 * events from being received by the application (MainFrame, its
	 * subcomponents and the menu bar).
	 * 
	 * @param enabled
	 *            <code>true</code> to enable 'no events mode',
	 *            <code>false</code> to disable it
	 */
	public void setNoEventsMode(boolean enabled) {
		// Piece of code used in 0.8 beta1 and removed after because it's way
		// too slow, kept here for the record
		// // Glass pane has empty mouse and key adapters (created in the
		// constructor)
		// // which will catch all mouse and keyboard events
		// getGlassPane().setVisible(enabled);
		// getJMenuBar().setEnabled(!enabled);
		// // Remove focus from whatever component in FolderPanel which had
		// focus
	...
	/**
	 * Returns the {@link ToolBar} where shortcut buttons (go back, go forward,
	 * ...) are. Note that a non-null instance of {@link ToolBar} is returned
	 * even if it is currently hidden.
	 * 
	...
	/**
	 * Returns the panel where the {@link ToolBar} component is. Note that a
	 * non-null instance of {@link ToolBar} is returned even if it is currently
	 * hidden.
	 * 
	 * @return the toolbar component
	 */
	...
	/**
	 * Returns the {@link CommandBar}, i.e. the component that contains
	 * shortcuts to certains actions such as View, Edit, Copy, Move, etc... Note
	 * that a non-null instance of {@link CommandBar} is returned even if it is
	 * currently hidden.
	 * 
	...
	/**
	 * Returns the status bar, where information about selected files and volume
	 * are displayed. Note that a non-null instance of {@link StatusBar} is
	 * returned even if it is currently hidden.
	 * 
	 ...
	 * 
	 * <p>
	 * The returned table doesn't necessarily have focus, the focus can be in
	 * some other component of the active {@link FolderPanel}, or nowhere in the
	 * MainFrame if it is currently not in the foreground.
	 * </p>
	 * 
	 * <p>
	 * Use {@link FileTable#hasFocus()} to test if the table currently has
	 * focus.
	 * </p>
	 * 
	 ...
	 * 
	 * <p>
	 * The returned panel doesn't necessarily have focus, for example if the
	 * MainFrame is currently not in the foreground.
	 * </p>
	 * 
	...
	/**
	 * Sets the currently active FileTable. This method is to be called by
	 * FolderPanel only.
	 * 
	 * @param table
	 *            the currently active FileTable
	 */
	void setActiveTable(FileTable table) {
		...
		if (activeTableChanged) {
			...
			// Fire table change events on registered ActivePanelListener
			// instances.
	...
	/**
	 * Returns the inactive table, i.e. the complement of
	 * {@link #getActiveTable()}.
	 * 
	...
	/**
	 * Returns the inactive panel, i.e. the complement of
	 * {@link #getActivePanel()}.
	 * 
	...
	/**
	 * Specifies how folder panels are split: if true is passed, the folder
	 * panels will be split vertically (default), horizontally otherwise.
	 * 
	 * @param vertical
	 *            if true, the folder panels will be split horizontally
	 *            (default), vertically otherwise.
	 */
	public void setSplitPaneOrientation(boolean vertical) {
		// Note: the vertical/horizontal terminology used in muCommander is just
		// the opposite of the one used
		// in JSplitPane which is anti-natural / confusing
		splitPane.setOrientation(vertical ? JSplitPane.HORIZONTAL_SPLIT
	...
	/**
	 * Returns how folder panels are currently split: if <code>true</code> is
	 * returned, panels are split vertically (default), horizontally otherwise.
	 * 
	...
	private void saveSplitPaneOrientation() {
		...
		MuConfiguration
				.setVariable(
						SPLIT_ORIENTATION,
						splitPane.getOrientation() == JSplitPane.HORIZONTAL_SPLIT ? MuConfiguration.VERTICAL_SPLIT_ORIENTATION
								: MuConfiguration.HORIZONTAL_SPLIT_ORIENTATION);
	}

	/**
	 * Swaps the two FolderPanel instances: after a call to this method, the
	 * left FolderPanel will be the right one and vice-versa.
	 */
	...
	public void setSameFolder() {
		(activeTable == leftTable ? rightTable : leftTable).getFolderPanel()
				.tryChangeCurrentFolder(activeTable.getCurrentFolder());
	}

	/**
	 * Returns <code>true</code> if this MainFrame is currently active in the
	 * foreground.
	 * 
	 * @return <code>true</code> if this MainFrame is currently active in the
	 *         foreground
	 */
	...
	/**
	 * Sets whether this MainFrame is currently active in the foreground. This
	 * method is to be called by WindowManager only.
	 * 
	 * @param foregroundActive
	...
	/**
	 * Returns <code>true</code> if this MainFrame is active, or is an ancestor
	 * of a Window that is currently active.
	 * 
	 * @return <code>true</code> if this MainFrame is active, or is an ancestor
	 *         of a Window that is currently active
	 */
	...
	/**
	 * Updates this window's title to show currently active folder and window
	 * number. This method is called by this class and WindowManager.
	 */
	public void updateWindowTitle() {
		...
		if (OsFamilies.MAC_OS_X.isCurrent()
			...
			// Displays the document icon in the window title bar, works only
			// for local files
			AbstractFile currentFolder = activeTable.getCurrentFolder();
			Object javaIoFile;
			if (currentFolder.getURL().getScheme().equals(FileProtocols.FILE)) {
				// If the current folder is an archive entry, display the
				// archive file, this is the closest we can get
				// with a java.io.File
				if (currentFolder.hasAncestor(AbstractArchiveEntryFile.class))
					javaIoFile = currentFolder.getParentArchive()
							.getUnderlyingFileObject();
				else
					javaIoFile = currentFolder.getUnderlyingFileObject();
			} else {
				// If the current folder is not a local file, use the special
				// /Network directory which is sort of
				// 'Network Neighborhood'.
				javaIoFile = new java.io.File("/Network");
			}

			// Note that for some strange reason (looks like a bug), setting the
			// property to null won't remove
			// the previous icon.
	...
	/**
	 * Overrides <code>java.awt.Window#dispose</code> to save last MainFrame's
	 * attributes in the preferences before disposing this MainFrame.
	 */
	@Override
	public void dispose() {
		// Save last MainFrame's attributes (last folders, window position) in
		// the preferences.
		...
		MuConfiguration
				.setVariable(MuConfiguration.LAST_X, (int) bounds.getX());
		MuConfiguration
				.setVariable(MuConfiguration.LAST_Y, (int) bounds.getY());
		MuConfiguration.setVariable(MuConfiguration.LAST_WIDTH,
				(int) bounds.getWidth());
		MuConfiguration.setVariable(MuConfiguration.LAST_HEIGHT,
				(int) bounds.getHeight());
		Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
		MuConfiguration.setVariable(MuConfiguration.SCREEN_WIDTH,
				screenSize.width);
		MuConfiguration.setVariable(MuConfiguration.SCREEN_HEIGHT,
				screenSize.height);
		...
		MuConfiguration.setVariable(MuConfiguration.LEFT_SORT_BY, leftTable
				.getSortInfo().getCriterion().toString().toLowerCase());
		MuConfiguration
				.setVariable(
						MuConfiguration.LEFT_SORT_ORDER,
						leftTable.getSortInfo().getAscendingOrder() ? MuConfiguration.SORT_ORDER_ASCENDING
								: MuConfiguration.SORT_ORDER_DESCENDING);
		MuConfiguration.setVariable(MuConfiguration.RIGHT_SORT_BY, rightTable
				.getSortInfo().getCriterion().toString().toLowerCase());
		MuConfiguration
				.setVariable(
						MuConfiguration.RIGHT_SORT_ORDER,
						rightTable.getSortInfo().getAscendingOrder() ? MuConfiguration.SORT_ORDER_ASCENDING
								: MuConfiguration.SORT_ORDER_DESCENDING);
		...
		MuConfiguration.setVariable(MuConfiguration.LEFT_TREE_VISIBLE,
				leftFolderPanel.isTreeVisible());
		MuConfiguration.setVariable(MuConfiguration.RIGHT_TREE_VISIBLE,
				rightFolderPanel.isTreeVisible());
		MuConfiguration.setVariable(MuConfiguration.LEFT_TREE_WIDTH,
				leftFolderPanel.getTreeWidth());
		MuConfiguration.setVariable(MuConfiguration.RIGHT_TREE_WIDTH,
				rightFolderPanel.getTreeWidth());
	...
	/**
	 * Overrides <code>java.awt.Window#toFront</code> to have the window return
	 * to a normal state if it is minimized.
	 */
	@Override
	public void toFront() {
	...
	protected class CustomFocusTraversalPolicy extends FocusTraversalPolicy {
		...
		public Component getComponentAfter(Container container,
			...
			if (component instanceof QuickListFocusableComponent) {
				return ((QuickListFocusableComponent) component)
						.getInvokerFileTable();
			} else if (component == leftFolderPanel.getFoldersTreePanel()
					.getTree()) {
				return leftTable;
			} else if (component == rightFolderPanel.getFoldersTreePanel()
					.getTree()) {
				return rightTable;
