...
/**
 * ActionManager provides methods to retrieve {@link MuAction} instances and
 * invoke them. It keeps track of all the action instances it has created and
 * allows them to be reused within a {@link MainFrame}.
 * <p>
 * MuAction subclasses should not be instantiated directly,
 * <code>getActionInstance</code> methods should be used instead. Using
 * ActionManager to retrieve a MuAction ensures that only one instance exists
 * for a given {@link MainFrame}. This is particularly important because actions
 * are stateful and can be used in several components of a MainFrame at the same
 * time; if an action's state changes, the change must be reflected everywhere
 * the action is used. It is also important for performance reasons: sharing one
 * action throughout a {@link com.mucommander.ui.main.MainFrame} saves some
 * memory and also CPU cycles as some actions listen to particular events to
...
public class ActionManager {
	...
	public static void registerActions() {
		...
		// registerAction(new GarbageCollectAction.Descriptor(), new
		// GarbageCollectAction.Factory());
		...
		// registerAction(new OpenLocationAction.Descriptor(), new
		// OpenLocationAction.Factory());
		...
		// registerAction(new ToggleColumnAction.Descriptor(), new
		// ToggleColumnAction.Factory());
	 ...
	 *            - ActionDescriptor instance of the action.
	 * @param actionFactory
	 *            - ActionFactory instance of the action.
	 */
	 ...
	 * 
	 * @param actionClassPath
	 *            - path to MuAction class.
	 * @return String representing the id of the MuAction in the specified path.
	 *         null is returned if the given path is invalid.
	 */
	 ...
	 * @param actionId
	 *            - id of MuAction.
	 * @return true if an MuAction which is represented by the given id is
	 *         registered, otherwise return false.
	 */
	...
	/**
	 * Convenience method that returns an instance of the action denoted by the
	 * given ID, and associated with the specified <code>MainFrame</code>. This
	 * method creates an ActionParameters with no initial property, passes it to
	 * {@link #getActionInstance(ActionParameters, MainFrame)} and returns the
	 * {@link MuAction} instance.
	 * 
	 * @param actionId
	 *            ID of the action to instantiate
	 * @param mainFrame
	 *            the MainFrame instance the action belongs to
	 * @return a MuAction instance matching the given action ID and MainFrame,
	 *         <code>null</code> if the action could not be found or could not
	 *         be instantiated.
	 */
	...
	/**
	 * Returns an instance of the MuAction class denoted by the given
	 * ActionParameters and for the specified MainFrame. If an existing instance
	 * corresponding to the same ActionParameters and MainFrame is found, it is
	 * simply returned. If no matching instance could be found, a new instance
	 * is created, added to the internal action instances map (for further use)
	 * and returned. If the action denoted by the specified ActionParameters
	 * cannot be found or cannot be instantiated, <code>null</code> is returned.
	 * 
	 * @param actionParameters
	 *            a descriptor of the action to instantiate with initial
	 *            properties
	 * @param mainFrame
	 *            the MainFrame instance the action belongs to
	 * @return a MuAction instance matching the given ActionParameters and
	 *         MainFrame, <code>null</code> if the MuAction action denoted by
	 *         the ActionParameters could not be found or could not be
	 *         instantiated.
	 */
	public static MuAction getActionInstance(ActionParameters actionParameters, MainFrame mainFrame) {
		...
		// Looks for an existing MuAction instance used by the specified
		// MainFrame
		if (mainFrameActions.containsKey(actionParameters)) {
			...
			// else clone the hashtable to ensure that it doesn't get modified
			// by action instances.
			// Since cloning is an expensive operation, this is done only if the
			...
			// If the action's label has not been set yet, use the action
			// descriptor's
			if (action.getLabel() == null) {
				// Retrieve the standard label entry from the dictionary and use
				// it as this action's label
				String label = ActionProperties.getActionLabel(actionId);

				// Append '...' to the label if this action invokes a dialog
				// when performed
				...
				// Looks for a standard label entry in the dictionary and if it
				// is defined, use it as this action's tooltip
				String tooltip = ActionProperties.getActionTooltip(actionId);
				if (tooltip != null)
					action.setToolTipText(tooltip);
			}

			// If the action's accelerators have not been set yet, use the ones
			// from ActionKeymap
			if (action.getAccelerator() == null) {
				// Retrieve the standard accelerator (if any) and use it as this
				// action's accelerator
				KeyStroke accelerator = ActionKeymap.getAccelerator(actionId);
				if (accelerator != null)
					action.setAccelerator(accelerator);

				// Retrieve the standard alternate accelerator (if any) and use
				// it as this action's alternate accelerator
				accelerator = ActionKeymap.getAlternateAccelerator(actionId);
				if (accelerator != null)
					action.setAlternateAccelerator(accelerator);
			}

			// If the action's icon has not been set yet, use the action
			// descriptor's
			if (action.getIcon() == null) {
				// Retrieve the standard icon image (if any) and use it as the
				// action's icon
	...
	/**
	 * Returns a Vector of all MuAction instances matching the specified action
	 * id.
	 * 
	 * @param muActionId
	 *            the MuAction id to compare instances against
	 * @return a Vector of all MuAction instances matching the specified action
	 *         id
	 */
	...
	/**
	 * Convenience method that retrieves an instance of the action denoted by
	 * the given ID and associated with the given {@link MainFrame} and calls
	 * {@link MuAction#performAction()} on it. Returns <code>true</code> if an
	 * instance of the action could be retrieved and performed,
	 * <code>false</code> if the MuAction could not be found or could not be
	 * instantiated.
	 * 
	 * @param actionId
	 *            ID of the action to perform
	 * @param mainFrame
	 *            the MainFrame the action belongs to
	 * @return true if the action instance could be retrieved and the action
	...
	/**
	 * Convenience method that retrieves an instance of the MuAction denoted by
	 * the given {@link ActionParameters} and associated with the given
	 * {@link com.mucommander.ui.main.MainFrame} and calls
	 * {@link MuAction#performAction()} on it. Returns <code>true</code> if an
	 * instance of the action could be retrieved and performed,
	 * <code>false</code> if the MuAction could not be found or could not be
	 * instantiated.
	 * 
	 * @param actionParameters
	 *            the ActionParameters of the action to perform
	 * @param mainFrame
	 *            the MainFrame the action belongs to
	 * @return true if the action instance could be retrieved and the action
	 *         performed, false otherwise
