...
public class LocationTextField extends ProgressTextField implements
		LocationListener, FocusListener, ThemeListener {
	/** FolderPanel this text field is displayed in */
	private FolderPanel folderPanel;

	/**
	 * True while a folder is being changed after a path was entered in the
	 * location field and validated by the user
	 */
	private boolean folderChangeInitiatedByLocationField;

	/**
	 * Used to save the path that was entered by the user after validation of
	...
	public LocationTextField(FolderPanel folderPanel) {
		// Use a custom text field that can display loading progress when
		// changing folders
		super(0, ThemeManager
				.getCurrentColor(Theme.LOCATION_BAR_PROGRESS_COLOR));

		this.folderPanel = folderPanel;

		// Applies theme values.
		setFont(ThemeManager.getCurrentFont(Theme.LOCATION_BAR_FONT));
		setDisabledTextColor(ThemeManager
				.getCurrentColor(Theme.LOCATION_BAR_FOREGROUND_COLOR));
		setForeground(ThemeManager
				.getCurrentColor(Theme.LOCATION_BAR_FOREGROUND_COLOR));
		setBackground(ThemeManager
				.getCurrentColor(Theme.LOCATION_BAR_BACKGROUND_COLOR));
		setSelectedTextColor(ThemeManager
				.getCurrentColor(Theme.LOCATION_BAR_SELECTED_FOREGROUND_COLOR));
		setSelectionColor(ThemeManager
				.getCurrentColor(Theme.LOCATION_BAR_SELECTED_BACKGROUND_COLOR));

		// Listen to location changes to update popup menu choices and disable
		// this component while the location is
		// being changed
		folderPanel.getLocationManager().addLocationListener(this);

		// Listen to focus events to temporarily disable the MainFrame's
		// JMenuBar when this component has the keyboard focus.
	...
	private void enableAutoCompletion() {
		new TextFieldCompletion(new AutocompleterTextComponent(this) {
			...
			public void OnEnterPressed(KeyEvent keyEvent) {
				...
				// /!\ Consume the event so to prevent JTextField from firing an
				// ActionEvent
				keyEvent.consume();
			}
	...
	/**
	 * Re-enable this text field after a folder change was completed, cancelled
	 * by the user or has failed.
	 * 
	 * <p>
	 * If the folder change was the result of the user manually entering a path
	 * in the location field and the folder change failed or was cancelled,
	 * keeps the path intact and request focus on the text field so the user can
	 * modify it.
	 */
	private void folderChangeCompleted(boolean folderChangedSuccessfully) {
		if (folderChangedSuccessfully || !folderChangeInitiatedByLocationField) {
			// Set the location field's contents to the new current folder's
			// path
		...
		// If the location was entered and validated in the location field and
		// the folder change failed or was cancelled...
		if (!folderChangedSuccessfully && folderChangeInitiatedByLocationField) {
			// Restore the text that was entered by the user
			setText(locationFieldTextSave);
			// Select the text to grab user's attention and make it easier to
			// modify
			selectAll();
	...
	public void locationChanging(LocationEvent e) {
		// Change the location field's text to the folder being changed, only if
		// the folder change was not initiated
		// by the location field (to preserve the path entered by the user while
		// the folder is being changed)
		if (!folderChangeInitiatedByLocationField) {
			...
			if (folderURL.getScheme().equals(FileProtocols.FILE)) {
				...
				// Under for OSes with 'root drives' (Windows, OS/2), remove the
				// leading '/' character
				if (LocalFile.hasRootDrives())
					locationText = PathUtils.removeLeadingSeparator(
							locationText, "/");
			}
		...
		// Disable component until the folder has been changed, cancelled or
		// failed.
		// Note: if the focus currently is in the location field, the focus
		// manager will release focus and give it
		// to the next component (i.e. FileTable)
		setEnabled(false);
	}

	public void locationChanged(LocationEvent e) {
		// Re-enable component and change the location field's text to the new
	...
	public void locationCancelled(LocationEvent e) {
		// Re-enable component and change the location field's text to the new
		// current folder's path.
		// If the path was entered in the location field, keep the path to give
	...
	public boolean textFieldValidated() {
		...
		// Save the path that was entered in case the location change fails or
		// is cancelled
		...
		// Look for a bookmark which name is the entered string (case
		// insensitive)
		...
		// Look for a system variable which name is the entered string (case
		// insensitive)
	...
	public void focusLost(FocusEvent e) {
		...
		// // lost then ensure location field's text is set to the current
		// directory.
