...
/**
 * IconManager takes care of loading, caching, rescaling the icons contained
...
public class IconManager {
	...
	private final static String                       BASE_IMAGE_FOLDER    = "/com/mucommander/res/images";
	/** Icon sets folders within the application's JAR file */
	private final static String                       ICON_SET_FOLDERS[]   = { BASE_IMAGE_FOLDER + "/file/", BASE_IMAGE_FOLDER + "/action/",
	        BASE_IMAGE_FOLDER + "/status_bar/", BASE_IMAGE_FOLDER + "/common/", BASE_IMAGE_FOLDER + "/preferences/", BASE_IMAGE_FOLDER + "/progress/",
	        BASE_IMAGE_FOLDER + "/language/", BASE_IMAGE_FOLDER + "/mucommander/" };

	static {
		// Initialize caches for icon sets that need it.
		// Icons which are displayed once in a while like preferences icons
	...
	/**
	 * Creates and returns an ImageIcon instance using the specified icon path
	 * and scale factor. No caching.
	 * 
	 * @param iconPath
	 *            path of the icon resource inside the application's JAR file
	 * @param scaleFactor
	 *            the icon scale factor, <code>1.0f</code> to have the icon in
	 *            its original size (no rescaling)
	 */
	public static ImageIcon getIcon(String iconPath, float scaleFactor) {
		URL resourceURL = IconManager.class.getResource(iconPath);
	...
	/**
	 * Convenience method, calls and returns the result of
	 * {@link #getIcon(String, float) getIcon(iconPath, scaleFactor)} with a
	 * scale factor of 1.0f (no rescaling).
	 */
	...
	/**
	 * Returns a scaled version of the given ImageIcon instance, using the
	...
	public static ImageIcon getScaledIcon(ImageIcon icon, float scaleFactor) {
		...
		return new ImageIcon(image.getScaledInstance((int) (scaleFactor * image.getWidth(null)), (int) (scaleFactor * image.getHeight(null)),
		        Image.SCALE_AREA_AVERAGING));
	}

	/**
	 * Returns a 'composite' icon made by composing the two given icons: the
	 * <code>backgroundIcon</code> is painted first, and the
	 * <code>foregroundIcon</code> is superposed, letting its non-transparent
	 * pixels reveal the background icon. For this method to provide a
	 * meaningful result, the two icons should have the same dimensions and the
	 * <code>foreground</code> should have some transparent pixels.
	 * 
	 * @param backgroundIcon
	 *            the icon that is painted first
	 * @param foregroundIcon
	 *            the icon that is superposed above backgroundIcon, should use
	 *            transparency
	...
	/**
	 * Returns an icon in the specified icon set and with the given name. If a
	 * scale factor other than 1.0f is passed, the return icon will be scaled
	 * accordingly.
	 * <p>
	 * If the icon set has a cache, first looks for an existing instance in the
	 * cache, and if it couldn't be found, create an instance and store it in
	 * the cache for future access. Note that the cached icon is unscaled, i.e.
	 * the scaled icon is not cached.
	 * </p>
	 * 
	 * @param iconSet
	 *            an icon set (see public constants for possible values)
	 * @param iconName
	 *            filename of the icon to retrieve
	 * @param scaleFactor
	 *            the icon scale factor, <code>1.0f</code> to have the icon in
	 *            its original size (no rescaling)
	 * @return an ImageIcon instance corresponding to the specified icon set,
	 *         name and scale factor, <code>null</code> if the image wasn't
	 *         found or couldn't be loaded
	 */
	...
	/**
	 * Convenience method, calls and returns the result of
	 * {@link #getIcon(int, String, float) getIcon(iconSet, iconName,
	 * scaleFactor)} with a scale factor of 1.0f (no rescaling).
	 */
	...
	/**
	 * Returns an icon made of the specified icon and some transparent space
	 * around it.
	 * 
	 * @param icon
	 *            the original icon, will be painted at the center of the new
	 *            icon
	 * @param insets
	 *            specifies the dimensions of the transparent space around the
	 *            returned icon
	 * @return an icon made of the specified icon and some transparent space
	 *         around it
	 */
	public static ImageIcon getPaddedIcon(ImageIcon icon, Insets insets) {
		BufferedImage bi = new BufferedImage(icon.getIconWidth() + insets.left + insets.right, icon.getIconHeight() + insets.top + insets.bottom,
	 ...
	 * This method is useful when an ImageIcon is needed and only an Icon is
	 * available.
	 * <p>
	 * If the given Icon is already an ImageIcon, the same instance is returned.
	 * If it is not, a new ImageIcon is created and returned.
	 */
	...
	/**
	 * Returns the path to the folder that contains the image resource files of
	 * the given icon set. The returned path is relative to the application JAR
	 * file's root and contains a trailing slash.
	 * 
	 * @param iconSet
	 *            an icon set (see public constants for possible values)
	 */
