...
public class DesktopManager {
	 ...
	 * <p>
	 * These operations are for internal use only, and cannot be registered
	 * through the {@link #registerOperation(String,int,DesktopOperation)}
	 * method.
	 * </p>
	 */
	 ...
	 * <p>
	 * These operations are treated with a lower priority than
	 * {@link #SYSTEM_OPERATION} ones, but higher than
	 * {@link #FALLBACK_OPERATION} ones. They are meant for plugin specific
	 * operations and are expected to be fairly reliable.
	 * </p>
	 */
	 ...
	 * <p>
	 * These operations will only ever be used if nothing else is available.
	 * They need only be a best-effort solution.
	 * </p>
	 */
	...
	/*
	 * Static initialisation. Bear in mind that adapters and operations are
	 * registered the 'wrong' way around: the earlier they are registered, the
	...
	static {
		...
		// The default desktop adapter must be registered first, as we only want
		// to use
		...
		// - check for 'configured' before 'guessed', as guesses are less
		...
		// Having 1.6 specific operations registered as the lowest priority
		// system
		// ones ensures that:
		// - they are executed after CommandXXX operations (important, since
		// CommandXXX
		// operations are user configurable).
		// - they are executed before any other operations (if available, they
		// will
		...
		innerRegisterOperation(OPEN_IN_FILE_MANAGER, SYSTEM_OPERATION,
				new CommandOpenInFileManager());
		innerRegisterOperation(OPEN, SYSTEM_OPERATION, new CommandOpen(false));

		// The only FALLBACK operation we have at the time of writing is for
		// OPEN,
	 ...
	 * <p>
	 * If <code>install</code> is set to <code>true</code>, this method might
	 * result in installing desktop specific data such as bookmarks, keyboard
	 * shortcuts...
	 * </p>
	 * 
	 * @param install
	 *            whether or not to install desktop specific information.
	 * @throws DesktopInitialisationException
	 *             if an error occured while initialising desktops.
	 */
	public static void init(boolean install)
			throws DesktopInitialisationException {
		DesktopAdapter current;

		// Browses desktop from the last registered to the first, to make sure
		// that
	 ...
	 * <p>
	 * If the {@link #init(boolean)} method wasn't called, the
	 * <code>DesktopManager</code> will find itself in a situation where it
	 * doesn't know which desktop it's running on. Calling this method ensures
	 * that, if a {@link DesktopAdapter} instance is required, we have at least
	 * the {@link DefaultDesktopAdapter} to work with.
	 * </p>
	 */
	 ...
	 * <p>
	 * Note that the later an adapter is registered, the higher its priority.
	 * Since all default adapters are registered at initialisation time, any
	 * call to this method will result in the new adapter to be checked before
	 * them.
	 * </p>
	 * 
	 * @param adapter
	 *            desktop adapter to register.
	 */
	 ...
	 */
	private static void innerRegisterOperation(String type, int priority,
			DesktopOperation operation) {
		Vector<DesktopOperation> container;

		// Makes sure we have a container for operations of the specified
		// priority.
	...
	public static void registerOperation(String type, int priority,
			DesktopOperation operation) {
	...
	// - Operation support -----------------------------------------------
	// -------------------------------------------------------------------
	private static Vector<DesktopOperation> getOperations(String type,
			int priority) {
		if (operations[priority] == null)
			return null;
	...
	private static DesktopOperation getSupportedOperation(String type,
			int priority, Object[] target) {
		DesktopOperation operation;
		Vector<DesktopOperation> container;
	...
	public static void executeOperation(String type, Object[] target)
	...
	public static String getOperationName(String type)
	...
	public static String getOperationName(String type, Object[] target)
			throws UnsupportedOperationException {
		DesktopOperation operation;
	...
	public static void browse(AbstractFile url) throws IOException,
			UnsupportedOperationException {
		executeOperation(BROWSE, new Object[] { url });
	}
	...
	public static void open(AbstractFile file) throws IOException,
			UnsupportedOperationException {
		executeOperation(OPEN, new Object[] { file });
	}
	...
	public static void openInFileManager(String file) throws IOException,
			UnsupportedOperationException {
		executeOperation(OPEN_IN_FILE_MANAGER, new Object[] { file });
	}

	public static void openInFileManager(AbstractFile file) throws IOException,
			UnsupportedOperationException {
		executeOperation(OPEN_IN_FILE_MANAGER, new Object[] { file });
	}

	private static String getFileManagerName(DesktopOperation operation)
			throws UnsupportedOperationException {
		if (operation == null)
			throw new UnsupportedOperationException();
		return operation.getName();
	...
	public static String getFileManagerName(File file)
			throws UnsupportedOperationException {
		return getFileManagerName(getSupportedOperation(OPEN_IN_FILE_MANAGER,
				new Object[] { file }));
	}

	public static String getFileManagerName(String file)
			throws UnsupportedOperationException {
		return getFileManagerName(getSupportedOperation(OPEN_IN_FILE_MANAGER,
				new Object[] { file }));
	}

	public static String getFileManagerName(AbstractFile file)
			throws UnsupportedOperationException {
		return getFileManagerName(getSupportedOperation(OPEN_IN_FILE_MANAGER,
				new Object[] { file }));
	}
	...
	// -------------------------------------------------------------------
	/**
	 * Returns an instance of the {@link com.mucommander.desktop.AbstractTrash}
	 * implementation that can be used on the current platform.
	 * 
	 * @return an instance of the AbstractTrash implementation that can be used
	 *         on the current platform, or <code>null</code> if none is
	 *         available.
	 */
	...
	/**
	 * Returns the object used to create instances of
	 * {@link com.mucommander.desktop.AbstractTrash}.
	 * 
	 * @return the object used to create instances of {@link AbstractTrash} if
	...
	/**
	 * Sets the object that is used to create instances of
	 * {@link com.mucommander.desktop.AbstractTrash}.
	 * 
	 * @param provider
	 *            object that will be used to create instances of
	 *            {@link com.mucommander.desktop.AbstractTrash}.
	 */
	...
	// -------------------------------------------------------------------
	/**
	 * Checks whether the specified <code>MouseEvent</code> is a left-click for
	 * this destop.
	 * <p>
	 * There are some cases where Java doesn't detect mouse events properly -
	 * for example, <i>CONTROL + LEFT CLICK</i> is a <i>RIGHT CLICK</i> under
	 * Mac OS X.<br>
	 * The goal of this method is to allow desktop to check for such
	 * non-standard behaviours.
	 * </p>
	 * 
	 * @param e
	 *            event to check.
	 * @return <code>true</code> if the specified event is a left-click for this
	...
	/**
	 * Returns the maximum interval in milliseconds between mouse clicks for
	 * them to be considered as 'multi-clicks' (e.g. double-clicks). The
	 * returned value should reflects the desktop's multi-click (or
	 * double-click) interval, which may or may not correspond to the one Java
	 * uses for double-clicks.
	 * 
	 * @return the maximum interval in milliseconds between mouse clicks for
	 *         them to be considered as 'multi-clicks'.
	 */
	 ...
	 * <p>
	 * The returned command must set the shell in its 'run script' mode. For
	 * example, for bash, the returned command should be
	 * <code>/bin/bash -l -c"</code>.
	 * </p>
	 * 
	...
	/**
	 * Returns <code>true</code> if the given file is an application file. What
	 * an application file actually is is system-dependent and can take various
	 * forms. It can be a simple executable file, as in the case of Windows
	 * <code>.exe</code> files, or a directory containing an executable and
	 * various meta-information files, like Mac OS X's <code>.app</code> files.
	 * 
	 * @param file
	 *            the file to test
