 ...
 * {@link AbstractFile} instance for a specified path or {@link FileURL}
 * location. <h3>Protocols</h3>
 * <p>
 * In order to allow the <code>com.mucommander.file</code> API to access new
 * file protocols, developers must create an implementation of
 * {@link AbstractFile} that handles that protocol and register it to
 * <code>FileFactory</code>. This registration requires an implementation of
 * {@link ProtocolProvider}, an instance of which will be passed to
 ...
 * <p>
 * In order to allow the <code>com.mucommander.file</code> API to access new
 * archive formats, developers must create an implementation of
 * {@link AbstractArchiveFile} that handles that format and register it to
 * <code>FileFactory</code>. This registration requires an implementation of
 * {@link ArchiveFormatProvider}, an instance of which will be passed to
 * {@link #registerArchiveFormat(ArchiveFormatProvider)}.
 * </p>
 * <p>
 * Built-in file file formats are:
 * <ul>
 * <li><code>ZIP</code>, registered to zip, jar, war, wal, wmz, xpi, ear, odt,
 * ods and odp files.</li>
 * <li><code>TAR</code>, registered to tar, tar.gz, tgz, tar.bz2 and tbz2 files.
 * </li>
...
public class FileFactory {
	...
	static {
		...
		// TarArchiveFile must match 'tar.gz'/'tar.bz2' files before
		// GzipArchiveFile/Bzip2ArchiveFile does.
		...
		// mugCommander
		registerArchiveFormat(new FindFileFormatProvider());
	 ...
	 * <p>
	 * If a {@link ProtocolProvider} was already registered to the specified
	 * protocol, it will automatically be unregistered.
	 * </p>
	 * <p>
	 * The <code>protocol</code> argument is expected to be the protocol
	 * identifier, without the trailing <code>://</code>. For example, the
	 * identifier of the HTTP protocol would be <code>http</code>. This
	 * parameter's case is irrelevant, as it will be stored in all lower-case.
	 * </p>
	 * <p>
	 * After this call, the various {@link #getFile(String) getFile} methods
	 * will be able to resolve files using the specified protocol.
	 * </p>
	 * <p>
	 ...
	 * 
	 * @param protocol
	 *            identifier of the protocol to register.
	 * @param provider
	 *            object used to create instances of files using the specified
	 *            protocol.
	...
	public static ProtocolProvider registerProtocol(String protocol,
		...
		// Note that the local file provider is also added to the provider
		// hashtable.
	 ...
	 * 
	 * @param protocol
	 *            identifier of the protocol whose provider should be
	 *            unregistered.
	 * @return the provider that has been unregistered, or <code>null</code> if
	 *         none.
	 */
	...
	/**
	 * Returns the protocol provider associated with the specified protocol
	 * identifer, or <code>null</code> if there is none.
	 * 
	 * @param protocol
	 *            identifier of the protocol whose provider should be retrieved.
	 * @return the protocol provider registered to the specified protocol
	 *         identifer, or <code>null</code> if none.
	 */
	...
	/**
	 * Returns <code>true</code> if the given protocol has a registered
	 * {@link ProtocolProvider}.
	 * 
	 * @param protocol
	 *            identifier of the protocol to test
	 * @return <code>true</code> if the given protocol has a registered
	 *         {@link ProtocolProvider}.
	 */
	 ...
	 * <p>
	 * All objects returned by the iterator's <code>nextElement()</code> method
	 * will be string instances. These can then be passed to
	 * {@link #getProtocolProvider(String) getProtocolProvider} to retrieve the
	 * associated {@link ProtocolProvider}.
	 * </p>
	 * 
	 ...
	 * 
	 * @param provider
	 *            the <code>ArchiveFormatProvider</code> to register.
	 */
	 ...
	 * <p>
	 * To unregister the provider of a particular archive format without knowing
	 * the associated provider instance, use
	 * {@link #getArchiveFormatProvider(String)} with a known archive filename
	 * to retrieve the provider instance. For example,
	 * <code>FileFactory.unregisterArchiveFormat(FileFactory.getArchiveFormatProvider("file.zip"))</code>
	 * will unregister the (first, if any) Zip provider.
	 * </p>
	 * 
	 * @param provider
	 *            the <code>ArchiveFormatProvider</code> to unregister.
	...
	/**
	 * Updates the <code>ArchiveFormatProvider</code> array to reflect the
	...
	/**
	 * Returns the first <code>ArchiveFormatProvider</code> that matches the
	 * specified filename, <code>null</code> if there is none. Note that if a
	 * filename matches the {@link java.io.FilenameFilter} of several registered
	 * providers, the first provider matching the filename will be returned.
	 * 
	 * @param filename
	 *            an archive filename that potentially matches one of the
	 *            registered <code>ArchiveFormatProvider</code>
	 * @return the first <code>ArchiveFormatProvider</code> that matches the
	 *         specified filename, <code>null</code> if there is none
	 */
	 ...
	 * <p>
	 * This method does not throw any IOException but returns <code>null</code>
	 * if the file could not be created.
	 * </p>
	 * 
	 * @param absPath
	 *            the absolute path to the file
	 * @return <code>null</code> if the given path is not absolute or incorrect
	 *         (doesn't correspond to any file) or if something went wrong
	 ...
	 *            if set to <code>true</code>, an IOException will be thrown if
	 *            something went wrong during file creation
	 * @return <code>null</code> if the given path is not absolute or incorrect
	 *         (doesn't correspond to any file)
	 * @throws java.io.IOException
	 *             and throwException param was set to <code>true</code>.
	...
	/**
	 * Returns an instance of AbstractFile for the given absolute path and use
	 * the given parent for the new file if not null. AbstractFile subclasses
	 * should as much as possible call this method rather than
	 * {@link #getFile(String)} because it is more efficient.
	 * 
	 * @param absPath
	 *            the absolute path to the file
	 * @param parent
	 *            the returned file's parent
	 * @return an instance of <code>AbstractFile</code> for the specified
	 *         absolute path.
	 * @throws java.io.IOException
	 *             if something went wrong during file or file url creation.
	 * @throws AuthException
	 *             if additionnal authentication information is required to
	 *             create the file
	 */
	 ...
	 * @param fileURL
	 *            the file URL
	 * @return the created file or null if something went wrong during file
	 *         creation
	 */
	...
	/**
	 * Creates and returns an instance of AbstractFile for the given FileURL and
	 * uses the specified parent file (if any) as the created file's parent.
	 * <p>
	 * Specifying the file parent if an instance already exists allows to
	 * recycle the AbstractFile instance instead of creating a new one when the
	 * parent file is requested.
	 * 
	 * @param fileURL
	 *            the file URL representing the file to be created
	 * @param parent
	 *            the parent AbstractFile to use as the created file's parent,
	...
	public static AbstractFile getFile(FileURL fileURL, AbstractFile parent,
		...
		// Extract every filename from the path from left to right and for each
		// of them, see if it looks like an archive.
		// If it does, create the appropriate protocol file and wrap it into an
		// archive file.
		while (pt.hasMoreFilenames()) {

			// Test if the filename's extension looks like a supported archive
			// format...
			// Note that the archive can also be a directory with an archive
			// extension
			if (isArchiveFilename(pt.nextFilename())) {
				// Remove trailing separator of file, some file protocols such
		...
		// Create last file if it hasn't been already (if the last filename was
		// not an archive), same routine as above
	...
	private static AbstractFile createRawFile(FileURL fileURL,
		...
		AbstractFile file;
		// Lookup the pool for an existing AbstractFile instance, only if there
		// are no instantiationParams.
		// If there are instantiationParams (the file was created by the
		// AbstractFile implementation directly, that is
		// by ls()), any existing file in the pool must be replaced with a new,
		// more up-to-date one.
		if (instantiationParams.length == 0) {
			// Note: FileURL#equals(Object) and #hashCode() take into account
			// credentials and properties and are
			// trailing slash insensitive (e.g. '/root' and '/root/' URLS are
			// one and the same)
		...
		// Special case for local files to avoid provider hashtable lookup and
		...
		if (scheme.equals(FileProtocols.FILE)) {
			...
			// Uncomment this line and comment the previous one to simulate a
			// slow filesystem
			// file = new DebugFile(file, 0, 50);
		}
		// Use the protocol hashtable for any other file protocol
		else {
			// If the specified FileURL doesn't contain any credentials, use
			// CredentialsManager to find
		...
		// Note: Creating an archive file on top of the file must be done after
		// adding the file to the pool,
		// this could otherwise lead to weird behaviors, for example if a
		// directory with the same filename
		// of a former archive was created, the directory would be considered as
		// an archive.
	...
	/**
	 * Returns a variation of the given filename, appending a pseudo-unique ID
	 * to the filename's prefix while keeping the same filename extension.
	 * 
	 * @param filename
	 *            base filename
	 */
	...
	/**
	 * Creates and returns a temporary local file using the desired filename. If
	 * a file with this name already exists in the temp directory, the
	 * filename's prefix (name without extension) will be appended an ID. The
	 * filename's extension will however always be preserved.
	 * <p>
	 * The returned file may be a {@link LocalFile} or a
	 * {@link AbstractArchiveFile} if the extension corresponds to a registered
	 * archive format.
	 * </p>
	 * 
	 * @param desiredFilename
	 *            the desired filename for the temporary file. If a file with
	 *            this name already exists in the temp directory, the filename's
	 *            prefix (name without extension) will be appended an ID, but
	 *            the filename's extension will always be preserved.
	 * @param deleteOnExit
	 *            if <code>true</code>, the temporary file will be deleted upon
	 *            normal termination of the JVM
	 * @return the temporary file, may be a LocalFile or an AbstractArchiveFile
	 *         if the filename's extension corresponds to a registered archive
	 *         format.
	 * @throws IOException
	...
	/**
	 * Creates a temporary file with a default filename. This method is a
	 * shorthand for {@link #getTemporaryFile(String, boolean)} called with a
	 * <code>null</code> name.
	 * 
	 * @param deleteOnExit
	 *            if <code>true</code>, the temporary file will be deleted upon
	...
	/**
	 * Returns the temporary folder, i.e. the parent folder of temporary files
	 * returned by {@link #getTemporaryFile(String, boolean)}.
	 * 
	...
	/**
	 * Returns true if the given filename's extension matches one of the
	 * registered archive formats.
	 * 
	 * @param filename
	 *            the filename to test
	 * @return <code>true</code> if the specified filename is a known archive
	...
	/**
	 * Tests based on the given file's extension, if the file corresponds to a
	 * registered archive format. If it does, an appropriate
	 * {@link AbstractArchiveFile} instance is created on top of the provided
	 * file and returned. If it doesn't (the file's extension doesn't correspond
	...
	public static AbstractFile wrapArchive(AbstractFile file)
		...
		// Comparing the filename against each and every archive extension has a
		// cost, so we only perform the test if
		// the filename contains a dot '.' character, since most of the time
		// this method is called with a filename that
	...
	/**
	 * Returns the default {@link com.mucommander.file.icon.FileIconProvider}
	 * instance. The default provider class (before
	 * {@link #setDefaultFileIconProvider(com.mucommander.file.icon.FileIconProvider)}
	 * is called) is platform-dependent and as such may vary across platforms.
	 * <p>
	 * It is noteworthy that the provider returned by this method is used by
	 * {@link com.mucommander.file.AbstractFile#getIcon()} to create and return
	 * the icon.
	 * </p>
	 * 
	...
	/**
	 * Sets the default {@link com.mucommander.file.icon.FileIconProvider}
	 * implementation.
	 * <p>
	 * It is noteworthy that the provider returned by this method is used by
	 * {@link com.mucommander.file.AbstractFile#getIcon()} to create and return
	 ...
	 * 
	 * @param fip
	 *            the new value for the default FileIconProvider
	 */
