...
class EmblemExtensionSignature(GObject.GObject, Nemo.InfoProvider):
...
class PagaVCS(GObject.GObject, Nemo.MenuProvider):
	def __init__(self):
	...
	def _get_all_items(self, toolbar, strFiles):
		lstItems = []
		if (not toolbar):
			folderitem = Nemo.MenuItem(name='Nautilus::pagavcs', label='PagaVCS', tip='PagaVCS subversion client', icon='pagavcs-logo')
			submenu = Nemo.Menu()
		...
		while (i < size):
			...
			#	item = Nemo.MenuItem('pagavcsseparator%d'%i, '––––––––––','')
			#	submenu.append_item(item)
			
			item = Nemo.MenuItem(name=menuItems[i]+actionNamePostfix, label=menuItems[i+1], tip=menuItems[i+2], icon=menuItems[i+3])			
